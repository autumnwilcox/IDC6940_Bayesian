...
-   Bayesian models require a fully complete dataset, so missing values must be resolved before model fitting.

-   MICE imputation restored sample size and reduced potential bias compared to complete-case analysis.

-   The final modeling dataset (adult_imp1) contains all predictors and the diabetes outcome with no missing values.

-   Normalized NHANES weights (`wt_norm`) allow the Bayesian model to approximate population-level inference.

-   This dataset is used for all Bayesian estimates, diagnostics, and posterior predictive checks throughout the analysis.
:::
...
![](figs/bmi_predicted_vs_observed.png){width="70%" fig-align="center"}

-   Each point represents an individual’s observed BMI versus the model’s posterior-predicted mean BMI.
-   Error bars reflect the 95% credible intervals for predicted BMI values.
-   Points closely aligned along the diagonal indicate that the model’s predictions match observed BMI values well.
-   Confirms that the Bayesian model accurately captures BMI distribution within the dataset.
