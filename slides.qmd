...
-   **θ (Parameters):**- Regression coefficients of predictors
...
-   ::: panel-tabset
    ...
    -   **Logit link** function 
        -   𝑃(𝑌=1) = intercept + 𝛽age_𝑐 + 𝛽bmi_𝑐 + 𝛽sex + 𝛽race
    ...
    **Eg: log odds = 0.69**
    ...
    -   Before seeing the data
        ...
        -   **Types of Priors**:
            -   Non-informative – no strong beliefs
            -   Weakly informative – realistic ranges
            -   Informative – based on prior research
            -   Conjugate – mathematically convenient
            -   Empirical – data-driven
            -   Hierarchical – for multilevel models shrinkage – regularization (LASSO, horseshoe)

    -   **Prior = A Seatbelt for the Model**\
        **With a prior → the model stays within realistic, scientifically meaningful ranges**
...
-   **DIQ010**: “Has a doctor told you that you have diabetes?”
...
MCMC convergence: the point where the information from the prior and the data (likelihood) has been fully combined, and the GIBBS sampler has reached the posterior distribution — and the chain is now drawing valid samples from that posterior.
...
-   **Burn in period:** **first 500–1,000 iterations** (early draws) that are **not representative** of the true posterior are discarded
...
-   **Stable posteriors**: precise estimates, high uncertainty, narrow distributions, smooth and unimodal (no multimodality)
...
Posterior draws = 4000 draws (4 chains × 1000 iterations) via MCMC gives a posterior distribution of predictors
...
-   ::: panel-tabset
    ...
    We draw **500 draws** of predicted outcomes for each observation and calculated the proportion of outcome (Diabetes = 1)\
    ...
    **Bars for y (observed) fall within the range of yrep (simulated):** **model predicts the overall diabetes prevalence well**
    ...
    Histogram: Posterior distribution of the population proportion of diabetes. x-axis: proportion of diabetics (1s), y-axis: frequency of posterior draws

    -   The highest bar is around 0.11 (11%) **Symmetric**, centered at \~0.11

    -   Spread: 95% credible interval is roughly 9%–13% diabetes prevalence
...
-   Model’s mean diabetes prevalence = 10.95% (95% CI: 8.5%–12.8%)
...
**logit(Diabetes**i**)=**β**0+**β**Age**i**+**β**BMI**i**+**β**Sex**i**+**β**Race**i ![](images/clipboard-3125514674.png){width="463"}
...
**Blue curve**: predicted probability (diabetes) across the BMI range; consistently high (≈1)\
**Red horizontal line**: Target probability of diabetes = 0.3.\
**Red vertical line**: targeted BMI, when the predicted probability of diabetes is closest to the target bmi probability (≈18).\
