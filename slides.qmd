...
-   Sampling via **Stan’s NUTS algorithm** [@hoffman2014nuts]
...
-   **DIQ_H** — diabetes questionnaire\
    Data from NHANES 2013–2014 [@nchs2014].
...
## Implementing Traditional Logistic Regression
...
-   **NUTS (No-U-Turn Sampler):** Stan’s Hamiltonian Monte Carlo algorithm that automatically adapts step size and trajectory length. **It uses gradient information to make larger, informed moves**, improving efficiency and reducing random walk behavior.
...
![](figs/bayes_fit_summary.png){width="50%"}
...
**Left:** Density Plots; **Right:** Trace Plots

![](figs/mcmc_density_trace.png){width="60%"}

-   **Density Plots:** Stable posteriors with precise estimates and high uncertainty, narrow distributions, smooth and unimodal.
-   **Trace Plots:** No upward or downward trend (no drift). Chains overlap and stay within the same band.

### Autocorrelation Plot

![](figs/acf_age_bmi.png){fig-align="center" width="55%"}

-   Rapid decay to zero shows.
-   Well mixed, independent draws, no relationship between parameters.
:::
...
-   **Female:**\
    b = –0.66 (SD = 0.10, 95% CrI: –0.86 to –0.47) → protective effect

-   **Mexican American:**\
    b = 0.69 (SD = 0.17, 95% CrI: 0.34 to 1.03) → increased risk

-   **Other Hispanic:**\
    b = 0.43 (SD = 0.25, 95% CrI: –0.07 to 0.89) → uncertain/slightly positive

-   **Non-Hispanic Black:**\
    b = 0.53 (SD = 0.15, 95% CrI: 0.23 to 0.83) → clear positive effect

-   **Other / Multiracial:**\
    b = 0.81 (SD = 0.19, 95% CrI: 0.45 to 1.18) → increased risk
:::
::::
...
![](figs/ppc_scatter_avg.png){fig-align="center"}
...
![](figs/odds_ratio_comparison.png){width="55%"}
...
![](figs/bayes_R2.png){fig-align="center"}
...
-   **Median predicted probability:** \~0.2
...
Even a relatively low BMI (\~18) corresponds to a **30% predicted diabetes probability**.

This suggests that **non-BMI factors** (age, sex, race) strongly influence risk for this profile.
:::
