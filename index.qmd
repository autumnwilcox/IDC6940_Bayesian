...
```{r}
#| label: tbl-adult
#| tbl-cap: !expr sprintf("Excerpt of the cleaned NHANES 2013–2014 adult cohort (age ≥ 20; N = 5,769) with derived and standardized variables.", format(adult_n, big.mark = ","))
...
Overall missingness was low (~`r round(mean(is.na(adult_eda$bmi) | is.na(adult_eda$diabetes_dx)) * 100, 1)`%).
Gaps were concentrated in `bmi` (n = 249) and `diabetes_dx` (n = 177), while demographic and design variables were complete.
This limited pattern of missingness is consistent with a Missing At Random (MAR) mechanism and likely reflects reduced participation in the physical examination component among certain adults.
...
(1) survey-weighted logistic regression to account for the NHANES complex sampling design,
(2) multiple imputation (MICE) to handle missing BMI values, and
(3) Bayesian logistic regression with weakly informative priors to quantify parameter uncertainty.
...
#| echo: true
#| include: true
#| fig-cap: "Structure of the analytic dataset used for regression modeling, showing variable names, types, and sample values (N = 5,349)."
...
#| echo: true
#| include: true
#| fig-cap: "Distribution of participants by sex (Male = 2,551; Female = 2,798) in the analytic cohort."
...
#| echo: true
#| include: true
#| fig-cap: "Race/ethnicity composition of the analytic cohort, with most participants identifying as Non-Hispanic White (n = 2,293) and Non-Hispanic Black (n = 1,101)."
...
#| echo: true
#| include: true
#| fig-cap: "Observed diabetes prevalence (binary outcome variable `diabetes_dx`), with 597 diagnosed cases (1 = Yes) and 4,752 non-diabetic participants (0 = No)."
...
#| label: tbl-svylogit
#| echo: true
#| fig-cap: "Survey-weighted logistic regression: odds ratios (OR) and 95% confidence intervals for diabetes diagnosis among adults (NHANES 2013–2014)."
...
```{r}
#| label: fig-ppc-mean
#| fig-cap: "Posterior predictive checks for mean of the binary outcome."
#| echo: true
#| cache: false
...
```{r}
#| label: fig-ppc-sd
#| fig-cap: "Posterior predictive checks for standard deviation of the binary outcome."
#| echo: true
#| cache: false
...
means and standard deviations closely match observed data.
