...
```{r}
#| label: tbl-mice
#| tbl-cap: "Multiple Imputation (MICE): pooled odds ratios (OR) and 95% confidence intervals for diabetes diagnosis after imputing missing BMI (m = 5; BMI via predictive mean matching, age via normal regression; outcome not imputed)."
knitr::kable(mi_or)
```

MICE addressed missingness in BMI using predictive mean matching (PMM) and in age using normal regression; diabetes status was not imputed (used as a predictor only). Categorical variables (sex, race3) were imputed via logistic/polytomous regression. We generated m = 5 imputations and pooled estimates with Rubin’s rules. The pooled results in @tbl-mice are directionally consistent with the design-based MLE model.
...
Note: NHANES exam weights are used as importance weights; design strata/PSUs are not modeled directly in the Bayesian fit.
...
grepl("^sex", term, ignore.case = TRUE) ~ "Male vs Female",
