...
to merging to ensure atomic columns suitable for design-based
...
### Study Design and Design-Based Analysis
...
(1) design-based logistic regression to account for the NHANES
    complex sampling design,

(2) Bayesian logistic regression with weakly informative priors to
    quantify parameter uncertainty.

### Design-Based Logistic Regression (Survey-Weighted)
...
#| label: tbl-svylogit
#| echo: true
#| tbl-cap: "Design-based logistic regression (survey-weighted): odds ratios (OR) and 95% confidence intervals for diabetes diagnosis among adults (NHANES 2013–2014)."
...
-   **Model Robustness:** Results are consistent with those from the
    design-based logistic regression (survey-weighted) model, confirming stability across imputation and weighting approaches.
-->

### Bayesian Logistic Regression

Bayesian logistic regression was used to quantify parameter uncertainty 
and compare posterior estimates with the design-based model. Weakly 
...
#| message: false
#| warning: false
#| tbl-cap: "Odds Ratios (95% CI) for Key Predictors Across Design-based and Bayesian Models."

# Combine design-based and Bayesian OR results
svy_tbl <- svy_or %>%
  mutate(Model = "Design-based (survey-weighted)")
...
knitr::kable(vertical_tbl,
             align = "lcc",
             caption = "Odds Ratios (95% CI) Across Design-based and Bayesian Models")
```
...
#| fig-cap: "Comparison of diabetes prevalence across design-based (NHANES), imputed, and posterior predictive estimates. The posterior predictive mean aligns closely with the observed NHANES prevalence, indicating strong model calibration."
#| echo: true
#| cache: false
#| include: true

# 1. Design-based prevalence
...
#| fig-cap: "Posterior predictive distribution of diabetes prevalence (solid density) overlaid with the design-based NHANES prevalence (vertical dashed line) and its 95% confidence interval (shaded band). The close overlap indicates that the Bayesian model accurately reproduces the observed population prevalence."
#| echo: true
#| cache: false
#| include: true
...
# Design-based observed prevalence (population estimate)
...
labs(
  ...
  y = "Posterior density",
  subtitle = sprintf("Design-based NHANES prevalence = %.1f%%", obs_prev * 100)
) +
theme_minimal()
```

The design-based NHANES diabetes prevalence was approximately
...
dplyr::mutate(svy_or,   Model = "Design-based (survey-weighted)") else NULL
...
#| echo: true
#| tbl-cap: "Comparison of odds ratios (per 1 SD for age and BMI) and 95% intervals across design-based and Bayesian frameworks."

knitr::kable(all_tbl, align = c("l","l","c"))
```

Across both frameworks—design-based (survey-weighted) and Bayesian—age and BMI were 
...
highly consistent with the design-based model.
...
replicate—design-based inference and may slightly affect standard error
estimation.
