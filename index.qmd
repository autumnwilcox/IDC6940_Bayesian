...
options(survey.lonely.psu = "adjust")  # safe default for single-PSU strata
...
# --- CLEAN, ATOMIC COPY FOR SURVEY CALCS (safe version; no unlist) ---
adult_clean <- adult %>%
  mutate(
    # ensure outcome is plain 0/1 numeric
    diabetes_ind = case_when(
      diabetes_ind %in% c(0L, 1L) ~ as.numeric(diabetes_ind),
      TRUE ~ NA_real_
    ),
...
# Guardrails
stopifnot(
  is.numeric(adult_clean$diabetes_ind),
  !is.list(adult_clean$sex),
  !is.list(adult_clean$race3),
  !is.list(adult_clean$WTMEC2YR)
)

# Extra: force factors to have their present levels only (helps svyby/svymean)
adult_clean <- adult_clean %>%
  mutate(
    sex   = factor(sex,   levels = unique(as.character(sex))) |> droplevels(),
    race3 = factor(race3, levels = unique(as.character(race3))) |> droplevels()
  )

str(adult_clean[, c("diabetes_ind","sex","race3","WTMEC2YR","SDMVPSU","SDMVSTRA")])
```

```{r}
#| label: qc-design-clean
#| echo: true
#| message: false
#| warning: false
...
nhanes_design_adult_clean <- survey::svydesign(
  id      = ~SDMVPSU,
  strata  = ~SDMVSTRA,
  weights = ~WTMEC2YR,
  nest    = TRUE,
...
# Quick guardrails
stopifnot(inherits(nhanes_design_adult_clean, "survey.design2"))
```
...
# Use the CLEAN design built on adult_clean
stopifnot(inherits(nhanes_design_adult_clean, "survey.design2"))

# 0) Overall prevalence (design-based) using svymean
des_outcome <- subset(nhanes_design_adult_clean, !is.na(diabetes_ind))

overall_est <- survey::svymean(~diabetes_ind, des_outcome, na.rm = TRUE)
overall_ci  <- confint(overall_est)[1, ]  # first (and only) coef
...
# Helper to compute prevalence by a factor without svyby
by_prevalence <- function(design, factor_name) {
  # extract non-missing levels from the underlying data in the design
...
by_sex_tbl <- by_prevalence(nhanes_design_adult_clean, "sex")
print(by_sex_tbl)

# 2) By race3
cat("\n--- Diabetes Prevalence by race3 ---\n")
by_race3_tbl <- by_prevalence(nhanes_design_adult_clean, "race3")
print(by_race3_tbl)
```
...
Male and White as reference levels, with race modeled using the
coarsened three-level factor (`race3`) (White, Black, Hispanic; smaller
groups lumped as ‘Other’ if present). Because some race/ethnicity
categories are sparse, race was coarsened to a three-level factor
(race3: White, Black, Hispanic; rare/other levels lumped as needed) to
reduce separation and improve stability. All analyses respect NHANES
...
# --- Design-based (survey) prevalence + 95% CI, using the CLEAN design ---

# Guardrail: diabetes_ind must be 0/1 numeric in adult_clean (set earlier)

des_outcome <- subset(nhanes_design_adult_clean, !is.na(diabetes_ind))

svy_mean <- svymean(~diabetes_ind, des_outcome, na.rm = TRUE)
pop_prev <- as.numeric(coef(svy_mean))          # proportion
pop_se   <- as.numeric(SE(svy_mean))            # SE of the proportion
pop_ci   <- c(pop_prev - 1.96 * pop_se, pop_prev + 1.96 * pop_se)
pop_ci   <- pmin(pmax(pop_ci, 0), 1)            # clamp to [0,1] for plotting
