...
sub-populations rather than simple random sampling. The Bayesian
...
- **Response Variable:**  
  `diabetes_dx` (binary) represents doctor-diagnosed diabetes. It was derived from `DIQ010` ("Doctor told you have diabetes"), with "Yes" coded as 1 and "No" coded as 0. "Borderline," "Refused," "Don't know," and missing responses were treated as missing.
...
var_tbl <- tribble(
  ...
  "diabetes_dx", "Doctor-diagnosed diabetes (1 = Yes, 0 = No); Borderline, Refused, Don't know, and missing responses treated as missing.", "Categorical", "Derived from DIQ010",
  ...
  "race", "Race/ethnicity categorized into five levels: Mexican American, Other Hispanic, Non-Hispanic White, Non-Hispanic Black, and Other/Multi-Racial.", "Categorical", "Derived from RIDRETH1",
...
A survey-weighted logistic regression (design-based) model was used to evaluate the association between diabetes status (`diabetes_dx`, binary outcome) and key predictors: body mass index (`bmi`), age (`age`), sex (`sex`), and race/ethnicity (`race`). Diabetes was defined using `DIQ010` (“Doctor told you have diabetes”) and coded as 0/1, with `DIQ010` used to define doctor-diagnosed diabetes; Borderline, Refused, Don't know, and missing responses were treated as missing.
...
`bmi_cat`)\
- `sex` (male, female)\
- `race` (five race/ethnicity levels: Mexican American, Other Hispanic, Non-Hispanic White, Non-Hispanic Black, Other/Multi-Racial)
...
This analysis uses data from the 2013–2014 cycle of the National Health and
Nutrition Examination Survey (NHANES), a nationally representative survey of
U.S. adults. Individuals aged **20 years and older** with available diabetes
outcome data and key demographic and anthropometric variables were included in
the analytic cohort.
...
The merged dataset contained **10,175 participants**. After restricting the sample to adults aged ≥20 years, **5,769 adults** remained. The primary outcome was **doctor-diagnosed diabetes** (`diabetes_dx`). Predictor variables included:
...
Survey design features—strata, primary sampling units, and sample weights—were
retained to support design-based logistic regression and ensure appropriate
...
adult <- merged_data %>%
  ...
  dplyr::mutate(
    ...
    # make NH White the reference level 
...
categories. These variables were standardized and used as predictors in subsequent modeling frameworks. The adult cohort included **5,769 participants aged ≥20 years**, of whom **5,592 had an observed diabetes outcome**.
...
The analytic adult cohort (N = `r format(adult_n, big.mark=",")`) includes standardized variables for age and BMI (`age_c`, `bmi_c`), categorical indicators for sex and race/ethnicity (`race`), and a binary doctor-diagnosed diabetes variable (`diabetes_dx`). This is a clean dataset with all NAs removed.
...
Overall missingness in the analytic variables was low, with approximately **3–4%** missing data in the outcome or BMI. Multiple imputation was used to address missing predictor data. Because the diabetes outcome was not imputed, participants with missing outcome data were excluded from outcome modeling.

#### Outcome Distribution

Doctor-diagnosed diabetes prevalence in the adult cohort was approximately
**11%**, which aligns with national estimates for U.S. adults during the same
time period.
...
- **BMI:** Individuals with diabetes tended to have higher BMI values, with
  clear separation between diabetes and non-diabetes groups.  
- **Sex:** Females exhibited slightly lower diabetes prevalence compared to
  males.  
- **Race/Ethnicity:** Prevalence was higher among Non-Hispanic Black, Hispanic,
  and “Other” racial/ethnic groups compared to Non-Hispanic White adults.  

These distributional patterns reflect established epidemiologic relationships
and support the suitability of the dataset for logistic regression modeling.
...
Two modeling frameworks were compared using identical
...
(2) Bayesian logistic regression with weakly informative priors to
...
Below is the structure of the **complete-case dataset** used for the survey-weighted frequentist regression analysis, showing variable names, types, and sample values (**N = 5,349**).
...
and compare posterior estimates with the survey-weighted model. Weakly 
informative priors were applied to regularize estimates while preserving 
...
Multiple Imputation by Chained Equations (`MICE`) was used to address missing data in the analytic dataset [@vanbuuren2011; @vanbuuren2012]. `MICE` iteratively imputes each incomplete variable using regression models based on the available data, producing multiple completed datasets that incorporate uncertainty from the imputation process. Estimates across imputations are then combined using Rubin’s rules to obtain final parameter estimates and confidence intervals.

As an alternative to full Bayesian joint modeling, `MICE` offers a flexible and robust approach for handling missingness through chained regression equations. For large sample sizes (`n ≥ 400`), even when a single variable contains substantial missingness (up to 75%), `MICE` performs reliably under skewed, multimodal, or heavy-tailed distributions without materially affecting mean-structure estimation [@vanbuuren2012].

In this study, Multiple Imputation by Chained Equations (`MICE`) was used to address missing predictor data. The analytic imputation dataset included `diabetes_dx`, age, BMI, sex, race/ethnicity, and the NHANES design variables `WTMEC2YR`, `SDMVPSU`, and `SDMVSTRA`. The outcome variable (`diabetes_dx`) was **not imputed**, but it was retained as a predictor for the imputation of other variables. Age was assigned normal linear regression, BMI predictive mean matching, and sex and race/ethnicity polytomous logistic regression. The NHANES examination weight, PSU, and stratum variables were not imputed but were included as auxiliary predictors. **Five imputed datasets** were generated (`m = 5`).

Because `diabetes_dx` was not imputed, participants with missing diabetes outcome data remained unavailable for outcome modeling. This resulted in **5,592 adults with an observed diabetes outcome** from the original adult cohort of 5,769. The original frequentist multiple-imputation analysis fit the logistic regression separately in all five completed datasets and pooled the estimates using Rubin's rules. In contrast, the current Bayesian analysis uses only the **first completed imputed dataset**; therefore, uncertainty across the five imputations is not propagated through the Bayesian model.
...
# Do NOT impute the outcome; allow it to predict other variables
meth["diabetes_dx"] <- ""
pred["diabetes_dx", ] <- 0     # no predictors used for diabetes_dx
pred[, "diabetes_dx"] <- 1     # diabetes_dx predicts other variables
diag(pred) <- 0                 # no variable predicts itself
...
#| echo: false
#| include: false
#| fig-cap: "Boxplot of BMI by diabetes status (`No` vs `Diabetes`). This descriptive plot compares BMI distributions between groups, highlighting higher median BMI, greater spread, and more outliers in the diabetes group. It is used in the EDA to summarize group differences before modeling."
...
<!--
The boxplot in Figure \@ref(fig-bmi-diabetes-box) is **descriptive**: it
summarizes the median, spread, and outliers in BMI for participants with
and without diabetes. The visibly higher median and wider spread in the
diabetes group reinforce the positive association between excess
adiposity and diabetes risk seen in later regression models.
-->
...
#| echo: false
#| include: false
#| fig-cap: "Predicted probability of diabetes as a function of BMI from a logistic regression model. This inferential plot visualizes the fitted relationship between BMI and diabetes risk: the smooth curve shows the modeled probability of diabetes, and the shaded band reflects uncertainty (95% confidence interval) around the fit."
...
**inferential**: it displays the regression-based fitted probability of
diabetes across the BMI continuum. The non-linear, increasing curve and
its confidence band summarize how modeled diabetes risk escalates with
higher BMI.
-->
...
-   **Model Robustness:** Results are consistent with those from the
    survey-weighted logistic regression model, confirming stability across imputation and weighting approaches.
-->

The Bayesian model was fit using the first completed imputed dataset and included 5,592 adults with an observed diabetes outcome. The survey-weighted frequentist analysis used a separate complete-case dataset of 5,349 adults. Therefore, the two models were not fit to the same analytic sample.
...
#| message: false
#| warning: false
#| tbl-cap: "Odds Ratios (95% CI) for Key Predictors Across Survey-weighted and Bayesian Models."
...
#| message: false
#| warning: false
#| fig-cap: "Coefficient estimates (log-odds) from survey-weighted and Bayesian logistic regression models for Age and BMI."
...
svy_df <- tidy(svy_fit, conf.int = TRUE) %>%
  filter(term %in% c("age_c", "bmi_c")) %>%
  mutate(
    Method = "Survey-weighted",
    term = recode(term, "age_c" = "Age (per 1 SD)", "bmi_c" = "BMI (per 1 SD)")
  )

# Bayesian coefficients
bayes_df <- tidy(bayes_fit, conf.int = TRUE) %>%
  filter(term %in% c("age_c", "bmi_c")) %>%
  mutate(
    Method = "Bayesian",
    term = recode(term, "age_c" = "Age (per 1 SD)", "bmi_c" = "BMI (per 1 SD)")
...
# Combine
coef_df <- bind_rows(svy_df, bayes_df) %>%
  mutate(Method = factor(Method, levels = c("Survey-weighted", "Bayesian")))
...
ggplot(coef_df, aes(x = term, y = estimate, color = Method)) +
  ...
  scale_color_manual(values = c("Survey-weighted" = "steelblue",
                                "Bayesian" = "darkred"))
```

The coefficient comparison plot shows that both modeling approaches identify Age and BMI as strong positive predictors of diabetes. The survey-weighted and Bayesian estimates are closely aligned, with overlapping intervals, indicating consistent effect sizes across methods. This agreement supports the robustness of the findings regardless of whether weighting or Bayesian regularization is used.
...
```{r}
#| label: fig-mcmc-pairs
#| fig-cap: "Posterior correlation among key regression parameters. Off-diagonal panels show pairwise joint posterior distributions, while diagonals show marginal posteriors. Weak correlations indicate predictors contribute independent information, supporting model stability."
#| echo: true
#| cache: false
...
#| fig-cap: "Calibration plot comparing observed diabetes outcomes (0/1) to model-predicted probabilities with a smoothed Locally Estimated Scatterplot Smoothing (LOESS) curve. The close alignment between the blue line and the diagonal (ideal calibration) indicates good model fit and reliable probability estimates."
#| echo: true
#| cache: false
#| include: true
...
#| fig-cap: "Comparison of diabetes prevalence across survey-weighted (NHANES) and posterior predictive estimates. The posterior predictive mean aligns closely with the observed NHANES prevalence, indicating strong model calibration."
#| echo: true
#| cache: false
#| include: true
...
pp_samples <- brms::posterior_predict(bayes_fit, ndraws = 1000)
pp_proportion <- rowMeans(pp_samples)

# 3. Build comparison table (imputed row removed)
...
The survey design-based estimate of doctor-diagnosed diabetes prevalence was approximately **8.9%**. In comparison, the Bayesian model produced a mean posterior predictive proportion of approximately **10.95%**. The Bayesian quantity is model-based and should not be interpreted as a design-based estimate of nationally representative prevalence.

```{r}
#| label: pop-vs-posterior-bar
#| fig-cap: "Population vs Posterior Diabetes Prevalence"
#| echo: true
#| include: true
#| eval: true
...
#| echo: true
#| cache: false
#| fig-cap: "Posterior predictive distribution for an example participant, showing the estimated probability of diabetes (P = 1) with 95% credible intervals (red dashed lines)."
...
#| echo: true
#| fig-cap: "Inverse prediction of BMI needed to reach a target diabetes probability (illustrative example)."
#| message: false
#| warning: false
...
profile. The “target BMI ≈ 18” marks the approximate threshold for a 30%
risk under this participant’s conditions.
...
#| echo: true
#| tbl-cap: "Comparison of odds ratios (per 1 SD for age and BMI) and 95% intervals across survey-weighted and Bayesian frameworks."

knitr::kable(all_tbl, align = c("l","l","c"))
```

Across both frameworks—survey-weighted (design-based) and Bayesian—age and BMI were 
consistently associated with higher odds of doctor-diagnosed diabetes. Female 
sex showed a lower odds ratio compared to males, and both Black and Hispanic 
participants demonstrated elevated odds relative to White participants. The 
similarity of effect sizes across frameworks underscores the robustness of 
these predictors to different modeling assumptions.
...
highly consistent with the survey-weighted model.
Age and BMI remained the most influential predictors
...
statements. The alignment between Bayesian and survey-weighted estimates
...
While this analysis highlights the strengths of Bayesian logistic
regression for epidemiologic modeling, several limitations should be
acknowledged.

First, the Bayesian model used a single imputed dataset rather than
jointly modeling imputation uncertainty, which may lead to a modest
understatement of total variance.

Second, NHANES examination weights were normalized and treated as
importance weights. This approach approximates—but does not fully
replicate—survey-weighted inference and may slightly affect standard error
estimation.

Third, the weakly informative priors $N(0, 2.5)$ for slopes and
Student-t(3, 0, 10) for the intercept were not empirically tuned.
Alternative prior choices could marginally shift posterior intervals or
reduce shrinkage.

Fourth, although convergence diagnostics (R̂ ≈ 1.00, large effective
...
and may not generalize to later survey waves or longitudinal
applications.

Finally, the model has not undergone external validation or formal
sensitivity analyses. The participant-level posterior risk estimates
presented here are intended for illustration only and should not be used
for individual prediction or clinical decision-making. Before external
deployment or use in other settings, the model would require validation
in independent datasets and sensitivity analyses to evaluate robustness
to both modeling assumptions and prior specifications.

#### Targeted Therapy

The Bayesian diabetes prediction project highlights the translational
potential of Bayesian modeling in clinical decision-making and public
health strategy.

By incorporating patient-level predictors such as age, BMI, sex, and
race to estimate diabetes probability, the model shifts from descriptive
statistics toward individualized risk prediction.

The translational value lies in converting these probabilistic outputs
into actionable thresholds—for example, identifying the BMI or age at
which predicted diabetes risk exceeds a clinically meaningful level
(e.g., 30%).

These insights can support early screening, personalized lifestyle
recommendations, and targeted prevention programs for higher-risk
populations.

This approach reflects precision public health by bridging data science
and clinical judgment to create tailored, evidence-based strategies that
may improve diabetes prevention and management outcomes.

**What changes in modifiable predictors would lower diabetes risk?**

#### Translational Research Implications

-   The model can be used to inform prevention and intervention
    strategies.

-   BMI is the only modifiable predictor in the current model.

-   Changes in BMI (via behavior or lifestyle interventions) can be
    explored to achieve a lower predicted risk.

-   Non-modifiable predictors (e.g., sex and race) are held constant.

-   The modifiable predictor (BMI) is varied until the model reaches the
...
magnitude to the frequentist results while offering richer uncertainty
quantification through posterior distributions, credible intervals, and
posterior predictive checks. Diagnostics (R̂ ≈ 1.00, large effective
sample sizes, and Bayesian R² ≈ 0.13) confirmed model convergence and
good fit, demonstrating that weakly informative priors effectively
regularized estimation without distorting inference.

Collectively, these results underscore the value of Bayesian inference
in epidemiologic research involving complex survey data. By integrating
prior information and leveraging MCMC sampling to approximate the full
posterior distribution, Bayesian models enhance transparency,
interpretability, and reproducibility. Future extensions could
incorporate hierarchical priors, combine multiple NHANES cycles, or
explore Bayesian model averaging to better capture population
