...
The multiple imputation (MICE) framework addressed missingness in BMI using predictive mean matching (PMM). Categorical variables (`sex`, `race3`) used logistic/polytomous regression. Results from multiple imputation were consistent with the survey-weighted model, confirming robustness to missing data handling. The imputation model used PMM for continuous variables, normal regression for age, and logistic/polytomous regression for categorical predictors, consistent with NHANES data structure and MICE best practices.
...
Posterior odds ratios and credible intervals from the Bayesian logistic regression are shown in Table @tbl-bayes.
...
Model-level performance is summarized in Table @tbl-bayesR2.
...
Calibration between predicted and observed diabetes probabilities is displayed in Figure @fig-pred-calibration.
