...
- Ecil Teodoro – Pushed literature review summaries to main branch.
...
Continuous predictors (BMI and age) were standardized (mean = 0, SD = 1) among adults aged ≥ 20 years prior to model estimation. Sex and race/ethnicity were recoded and re-leveled using the `forcats` package to ensure consistent reference categories (Male and non-Hispanic White). Analyses accounted for NHANES’s complex survey design using weights (`WTMEC2YR`), strata (`SDMVSTRA`), and primary sampling units (`SDMVPSU`). The survey-weighted model used a complete-case subset filtered on all required variables, while multiple imputation addressed missing predictor data.
...
Model objects and summary tables were saved as `.rds` files for reproducibility (`outputs/` directory). All analytic code was executed in R (version 4.3.2) using RStudio within a reproducible Quarto workflow.
