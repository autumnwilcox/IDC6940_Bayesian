...
Diabetes mellitus (DM) is a major public health challenge, and identifying key risk factors—such as obesity, age, sex, and race/ethnicity—is essential for prevention and targeted intervention. Logistic regression is widely used to estimate associations between such risk factors and binary outcomes, including the presence or absence of diabetes. However, classical maximum likelihood estimation (MLE) can yield unstable results in small samples or in the presence of missing data, quasi-separation, or complete separation. Moreover, healthcare data (e.g., DNA sequences, imaging, patient-reported outcomes, electronic health records, and longitudinal health measurements) are often complex, making standard analytical approaches insufficient [@zeger2020]. Recent Bayesian advances emphasize representing predictive **uncertainty** alongside point estimates, which is critical when clinical decisions hinge on risk thresholds; for example, approximate Bayesian deep learning with Monte Carlo dropout provides calibrated uncertainty without changing model architecture [@gal2016dropout]. In epidemiologic modeling, Bayesian approaches are increasingly preferred when separation, small samples, or missingness complicate classical inference [@baldwin2017; @kruschke2017].
...
In meta-analytic settings, Bayesian hierarchical regression has been used to augment data with results from prior studies. This approach incorporates both exchangeable and unexchangeable predictors, enabling explicit testing of heterogeneity across studies [@deleeuw2012]. Other works extend Bayesian methods to psychology and neuroscience (e.g., EEG studies [@baldwin2017]), intuitive reasoning frameworks [@kruschke2017], and Bayesian deep learning in healthcare applications such as imaging [@abdullah2022bdlhealth].

### Related Bayesian Methods for Clinical Risk Modeling

Beyond standard Bayesian GLMs, several strands of the literature inform our approach and possible extensions. **Bayesian nonparametric regression** can flexibly capture heavy tails and zero-inflation common in health cost and utilization data [@richardson2018bnr]. For **variable selection and nonlinear effects**, Bayesian Additive Regression Trees (BART) provide a robust alternative to linear terms; recent work proposes permutation- and backward-selection procedures that improve the identification of relevant predictors in mixed-type data [@luo2024bartvs]. In **dynamic risk prediction**, Bayesian state-space/Markov models that incorporate time-varying biomarkers have substantially improved clinical stratification relative to static baselines [@momeni2021covidbayes]. For **time-to-event outcomes**, BART-based survival models relax proportional hazards assumptions while delivering calibrated uncertainty [@sparapani2020bartsurvival]. Finally, **Bayesian model averaging** offers a principled way to account for model uncertainty by weighting across candidate specifications rather than selecting a single “best” model [@hoeting1999bma], and **Bayesian deep learning** contributes scalable uncertainty quantification when richer feature sets (e.g., imaging, EHR embeddings) are used [@abdullah2022bdlhealth; @gal2016dropout].
...
4. **Bayesian Logistic Regression:**  
   ...
   - Four chains × 2000 iterations (50% warm-up), `adapt_delta = 0.95`. Sampling used the No-U-Turn Sampler (NUTS), an adaptive variant of Hamiltonian Monte Carlo that removes most manual tuning while maintaining efficient exploration of complex posteriors [@hoffman2014nuts].
