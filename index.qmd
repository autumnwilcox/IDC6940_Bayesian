...
logit{P(Yi​=1)}=β0​+β1​BMIi​+β2​Agei​+β3​Sexi​+g∑​β3+g​Raceig​.

**BMI** and **Age** are standardized (mean 0, SD 1). Categorical
predictors use **Male** and **White** as reference levels. Because some
race/ethnicity categories are sparse, race was **coarsened** to a
...
diabetes_dx∼age_c+bmi_c+sex+race3.\texttt{diabetes\_dx} \sim \texttt{age\_c} + \texttt{bmi\_c} + \texttt{sex} + \texttt{race3}.diabetes_dx∼age_c+bmi_c+sex+race3.
...
intercept), sampling with **NUTS** (4 chains × 2000 iterations;
...
**Diagnostics.** Convergence and fit were acceptable
(\$\\hat{R}\<1.01\$, large effective sample sizes). Posterior predictive
checks (`pp_check`) were satisfactory; we also report Bayesian \$R\^2\$.
...
Across survey-weighted MLE, MI-pooled, and Bayesian models, **age** and
**BMI** showed **positive associations** with diabetes risk. **When
retained**, **female sex** tended to have **lower odds** versus males.
Using the coarsened race factor, **Black and Hispanic** groups generally
showed **higher odds relative to White**; very small categories were
lumped to avoid instability. Agreement in direction and approximate
magnitude across frameworks suggests robustness to listwise deletion,
imputation, and weak prior regularization.

We summarize the ORs (per 1 SD) and 95% intervals for **BMI** and
**Age** across the three modeling strategies; **sex** and **race3**
effects are shown when those factors were retained in the final model.
...
kable(
  res_wide, align = c("l","c","c"),
  caption = "Odds ratios (per 1 SD) with 95% intervals across models (terms shown only when retained in the final model)."
)
```
...
1.  **Cross-sectional design** limits causal inference, and
    **self-reported diabetes** (`DIQ010`) may be affected by recall or
    reporting bias.

2.  Although **multiple imputation** helped mitigate missing data, it
    assumes **MAR**; violations (e.g., **MNAR**) could bias results
    \[\@vanbuuren2012\].

3.  Our **Bayesian specification approximated** NHANES’s complex survey
    design by using **normalized weights** rather than embedding strata
    and PSUs directly in the likelihood, so variance may **under- or
    over-state** design effects \[\@nchs2014\].

4.  **BMI** was measured once and may not reflect long-term adiposity or
    metabolic trajectories.

5.  We modeled a **limited covariate set**, excluding factors such as
    diet, physical activity, socioeconomic status, and family history
    that may confound observed associations.

6.  **Coarsening and adaptive omission of factors.** Race/ethnicity was
    coarsened; when a factor **collapsed to one level** in an analysis
    subset, it was **omitted**. This reduces granularity and may
    **attenuate** between-group contrasts relative to a full five-level
    race specification.

7.  The **Bayesian model was fit on a single valid imputed dataset**
    (coarsened as needed). While sensitivity checks produced similar age
    and BMI effects, intervals may be **slightly narrower** than those
    from a full multiple-imputation Bayesian analysis.

Despite these limitations, the results provide robust and interpretable
estimates of diabetes risk factors and demonstrate the stability and
practical value of Bayesian inference in epidemiologic modeling
\[\@vandeschoot2021; \@nchs2014\].
...
imputation and prior choices, see @vanbuuren2012; @vandeschoot2021. For
survey design and weighting details, see @nchs2014.
