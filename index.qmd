...
| **Ethnicity Recoding** | `RIDRETH1` recoded as: 1 = Mexican American, 2 = Other Hispanic, 3 = Non-Hispanic White, 4 = Non-Hispanic Black, 5 = Other/Multi, then collapsed to four analytical levels (`race`): White, Black, Hispanic, Other. |
...
adult <- merged_data %>%
  ...
  dplyr::mutate(
    ...
    # collapse to 4-level race, same as adult_eda
    race = forcats::fct_collapse(
      race_raw,
      White    = "NH White",
      Black    = "NH Black",
...
tbl1_dat <- adult %>%
  transmute(
    ...
    sex,
    race,
    # CHANGE: make diabetes_dx a factor so it plays nice with pivot_longer()
...
kable(cat_sum,  caption = "Table 1b. Categorical variables (sex, race, diabetes_dx, bmi_cat): counts and percentages.")
```
...
race/ethnicity (`race`), and a binary doctor-diagnosed diabetes
variable (`diabetes_dx`).
...
sum(is.na(adult_eda$diabetes_dx))),
Missing_pct = round(c(mean(is.na(adult_eda$bmi)),
mean(is.na(adult_eda$diabetes_dx))) * 100, 1)
)
...
Missingness was low overall (~`r round(mean(is.na(adult_eda$bmi) | is.na(adult_eda$diabetes_dx)) * 100, 1)`%). Gaps were concentrated in `bmi` and `diabetes_dx`; design variables and demographics were complete. Patterns were consistent with MAR.
...
miss_diab <- mean(is.na(adult_eda$diabetes_dx)) * 100
sprintf("Missing BMI: %.1f%%; Missing diabetes_dx: %.1f%%", miss_bmi, miss_diab)
```
...
```{r}
#| label: corr-heatmap-eda
#| echo: true
#| fig-cap: "Correlation heatmap (EDA, pairwise-complete) using adult_eda."
stopifnot(all(c("diabetes_dx","age","bmi") %in% names(adult_eda)))

adult_eda_num <- adult_eda |>
  dplyr::transmute(
    diabetes_dx = as.numeric(diabetes_dx),
    age  = as.numeric(age),
    bmi  = as.numeric(bmi)
  )
...
(`diabetes_dx`). All design variables (`WTMEC2YR`, `SDMVPSU`,
`SDMVSTRA`), as well as `age`, `sex`, and `race`, are complete—sex and
...
(standardized age, BMI, sex, and race) and the binary outcome
...
| Predictor | Odds Ratio (OR) | p-value | Interpretation |
|------------|----------------:|---------:|----------------|
| **age_c** | 2.98 | <0.001 | Each 1 SD increase in age nearly triples the odds of diabetes. |
| **bmi_c** | 1.93 | <0.001 | Each 1 SD increase in BMI raises the odds of diabetes by about 93%. |
| **sexMale** | 1.29 | 0.037 | Males have about 29% higher odds of diabetes compared with females. |
| **raceHispanic** | 1.81 | <0.001 | Hispanic adults have ~81% higher odds of diabetes compared with Non-Hispanic Whites. |
| **raceBlack** | 1.60 | 0.009 | Non-Hispanic Black adults have ~60% higher odds of diabetes compared with Non-Hispanic Whites. |
| **raceOther** | 2.20 | 0.002 | Adults identifying as Other/Multi-racial have more than 2× higher odds of diabetes compared with Non-Hispanic Whites. |
...
In this study, continuous variables were imputed using regression-based methods: age via normal linear regression (norm) and BMI via predictive mean matching (PMM) to better preserve the empirical BMI distribution.
Categorical variables (sex and race) were imputed using logistic
and polytomous regression models, respectively. Diabetes status
(`diabetes_dx`) was treated as an outcome variable and was not imputed.
...
The chained equation process showed stable convergence across iterations,
indicating reliable estimation of missing BMI (and, where present, age) values. After MICE,
the final imputed dataset consisted of *n = 5,592 adults* with all key
predictor variables completed.
...
# Categoricals: sex = logistic; race = polytomous regression.
meth[c("age","bmi")]   <- c("norm","pmm")
meth[c("sex","race")] <- c("logreg","polyreg")
...
fits <- with(
  imp,
  glm(
    diabetes_dx ~ scale(age) + scale(bmi) + relevel(sex, "Male") + relevel(race, "White"),
    family = binomial()
  )
)
...
adult_imp1 <- mice::complete(imp, 1) %>%
  dplyr::mutate(
    # make sure factors line up with adult
    sex   = factor(sex,   levels = levels(adult$sex)),
    race  = factor(race,  levels = levels(adult$race)),

    # re-standardize in the imputed data
    age_c = as.numeric(scale(age)),
    bmi_c = as.numeric(scale(bmi)),
...
After imputation, the analytic dataset contained approximately 5,500–5,600 adults.
The mean age was around 49 years (SD ≈ 17), and the mean BMI was approximately 29 (SD ≈ 7).
Females represented roughly 52% of participants, and the majority identified as Non-Hispanic White (~43%).
Diabetes prevalence was about 11%, consistent with population-level NHANES estimates.
...
and compare posterior estimates with the survey-weighted and MICE models.
Weakly informative priors were used to regularize estimates while
preserving flexibility in inference.
...
#| message: false
#| warning: false
#| fig-cap: "Distributions for standardized BMI and Age (adult_imp1)."
...
#| message: false
#| warning: false
#| fig-cap: "Prior distributions for Age and BMI coefficients (Normal(0, 2.5))."
...
(elpd), confirming that both variables contribute meaningfully to model
fit.
...
#| echo: true
#| cache: false
#| fig-cap: "Posterior predictive check: density overlay for diabetes outcome."
...
Posterior predictive checks show excellent model calibration: simulated
means and standard deviations closely match observed data. The model
accurately reproduces diabetes prevalence (~10–11%) seen in the
imputed dataset.
...
Trace, density, and autocorrelation plots confirm smooth mixing,
unimodal posteriors, and minimal autocorrelation across chains.
...
```{r}
#| label: corr-heatmap-imputed-adult
#| echo: true
#| include: true
#| cache: false
#| fig-cap: "Correlation heatmap (imputed dataset: adult_imp1)."

stopifnot(all(c("diabetes_dx","age","bmi") %in% names(adult_imp1)))

corr_matrix_imputed <- cor(
adult_imp1[, c("diabetes_dx", "age", "bmi")],
use = "complete.obs",
method = "pearson"
)
...
want <- c("b_age_c","b_bmi_c","b_sexFemale","b_raceBlack","b_raceHispanic","b_raceOther")
...
obs_prev  <- mean(adult_imp1$diabetes_dx, na.rm = TRUE)
...
```{r}
#| label: prevalence-hist
#| echo: true
#| cache: false
#| fig-cap: "Posterior distribution of proportion of Diabetes = 1."
...
#| echo: true
#| cache: false
#| tbl-cap: "Comparison of Diabetes Prevalence Across Methods."
...
# (40-year-old Female, Hispanic)

library(dplyr)
library(ggplot2)

# 1. Grid of raw BMI values to explore
bmi_seq <- seq(18, 40, by = 0.5)

# 2. Match the standardisation used in the model
bmi_mean <- mean(adult_imp1$bmi, na.rm = TRUE)
bmi_sd   <- sd(adult_imp1$bmi,  na.rm = TRUE)

age_mean <- mean(adult_imp1$age, na.rm = TRUE)
age_sd   <- sd(adult_imp1$age,  na.rm = TRUE)

# Age 40 on the z-scale used in the model
age40_c <- (40 - age_mean) / age_sd

# 3. Prediction grid for one profile
newdata_grid <- tibble(
  age_c   = age40_c,
  bmi_c   = (bmi_seq - bmi_mean) / bmi_sd,            # z-score BMI
  bmi_raw = bmi_seq,
  sex     = factor("Female",   levels = levels(adult_imp1$sex)),
  race    = factor("Hispanic", levels = levels(adult_imp1$race)),
  wt_norm = 1
)

# 4. Simulate binary outcomes from the posterior (like Namita)
set.seed(1234)  # <-- important so you and Namita get the SAME line

pp_draws <- brms::posterior_predict(
  bayes_fit,
  newdata = newdata_grid,
  ndraws  = 1          # one posterior draw → 0/1 at each BMI
)

pred_df <- newdata_grid |>
  mutate(prob_mean = colMeans(pp_draws))   # will be 0 or 1 with ndraws = 1

# 5. Choose a target probability and find closest BMI
target_prob <- 0.30  # 30% risk
