...
As shown in @tbl-adult, the analytic adult cohort (N = `r format(adult_n, big.mark=",")`) includes standardized variables for age and BMI (`age_c`, `bmi_c`), categorical indicators for sex and race/ethnicity (`race3`), and a binary doctor-diagnosed diabetes variable (`diabetes_ind`). 
...
```{r}
#| label: tbl-mice
#| tbl-cap: "Multiple Imputation (MICE): pooled odds ratios (OR) and 95% confidence intervals for diabetes diagnosis after imputing missing BMI and diabetes values (m = 5, predictive mean matching)."
knitr::kable(mi_or)
```

The multiple imputation (MICE) framework was used to address missing
data in BMI and diabetes status. Continuous variables (`age`, `bmi`)
were imputed using normal regression for `age` and predictive mean
matching (PMM) for `bmi`, while categorical variables (`sex`, `race3`)
used logistic or polytomous regression.

Five imputed datasets (m = 5) were generated and analyzed separately;
estimates were then pooled using Rubin’s rules to obtain combined
parameter estimates and standard errors.

The pooled results, in @tbl-mice, show strong positive associations
between age and BMI with diabetes risk, as well as higher odds for males
and non-White racial/ethnic groups relative to Whites. The direction and
magnitude of effects are consistent with the design-based MLE model,
confirming that imputation did not meaningfully alter the relationships
among variables.
...
As shown in @tbl-bayes, this Bayesian logistic regression model estimates the log-odds of diabetes using standardized predictors.\

Weakly informative priors (Normal(0, 2.5) for slopes; Student-t(3, 0, 10) for the intercept) help stabilize estimation and prevent overfitting.\

Posterior means and 95% credible intervals (CrI) provide full uncertainty quantification for each predictor, enabling direct probabilistic interpretation.
...
#| tbl-cap: "MCMC diagnostics (R-hat and Effective Sample Sizes) for slope parameters."
#| echo: false
...
dplyr::mutate(bayes_or, Model = "Bayesian") else NULL
...
grepl("sex", term, ignore.case = TRUE) ~ "Female vs Male",
grepl("race3", term, ignore.case = TRUE) ~ sub("race3:","Race = ",term),
TRUE ~ term
),
...
The results across all modeling frameworks—survey-weighted, multiple
imputation, and Bayesian—were remarkably consistent, reinforcing the
robustness of associations between key demographic and anthropometric
factors and diabetes prevalence in the U.S. adult population.

Age and body mass index (BMI) emerged as the most influential predictors
of doctor-diagnosed diabetes, each showing positive and statistically
credible effects across models.\

These findings align with prior NHANES-based and epidemiologic
literature demonstrating the dose-dependent relationship between
adiposity, aging, and metabolic disease risk \[\@liu2013; \@nchs2014\].

The **survey-weighted model** captured the NHANES complex sampling
design, producing population-representative estimates.\

The **MICE-pooled model** confirmed that handling missing data via
multiple imputation yielded similar effect sizes, suggesting that
missingness did not meaningfully bias results.\

The **Bayesian logistic regression** model, while using normalized
weights as importance weights rather than explicit strata/PSUs, produced
odds ratios nearly identical in magnitude and direction to the
design-based estimates.\

Bayesian credible intervals were slightly narrower, reflecting
regularization by weakly informative priors and richer posterior
uncertainty quantification.

From a methodological standpoint, Bayesian logistic regression offered
several advantages:\

(1) flexibility in incorporating prior information,\

(2) interpretability through posterior probabilities, and\

(3) improved stability under sparse or imbalanced data.\

The close agreement between Bayesian and traditional frameworks
underscores the reliability of these associations while demonstrating
the practical value of Bayesian inference in public health modeling.

In conclusion, these results reaffirm that **age** and **BMI** remain
dominant, independent predictors of diabetes risk among U.S. adults,
with sex and race contributing additional variation.\

The consistency across analytic approaches highlights the robustness of
the findings and supports Bayesian logistic regression as a modern,
interpretable, and computationally feasible method for complex survey
data when uncertainty quantification is central to inference.
