...
We apply Bayesian logistic regression to predict diabetes status and to
estimate the associations between body mass index (BMI), age (≥ 20
...
NHANES files. 2. Define outcome and predictors. 3. Standardize
continuous predictors for numerical stability. 4. Specify the NHANES
survey design with proper weights, strata, and PSUs. 5. Perform
exploratory visualization and missing data checks. 6. Save cleaned
datasets for reproducible downstream modeling.
...
-   **Diabetes Outcome Clarification**: NHANES 2013–2014 lacks a
    distinct gestational-diabetes variable. Because `DIQ010` asks
    whether a doctor diagnosed diabetes “other than during pregnancy,”
    cases limited to pregnancy are automatically excluded.
...
# --- Import and COERCE inside each source BEFORE join ---
...
diq_h <- nhanesA::nhanes("DIQ_H") %>%
  transmute(
    SEQN,
    DIQ010 = coerce_num(DIQ010)  # doctor-diagnosed diabetes (other than during pregnancy)
  )

# --- Join on already-coerced tables ---
merged_data <- demo_h %>%
...
# Build adult with HARD type coercions and no list-cols
adult <- merged_data %>%
  dplyr::filter(RIDAGEYR >= 20) %>%
  dplyr::transmute(
    SDMVPSU, SDMVSTRA, WTMEC2YR,
    # Outcome numeric 0/1/NA
    diabetes_ind = dplyr::case_when(
      DIQ010 == 1 ~ 1,
      DIQ010 == 2 ~ 0,
    ...
    sex0 = dplyr::case_when(
      RIAGENDR == 1 ~ "Male",
      RIAGENDR == 2 ~ "Female",
      TRUE ~ NA_character_
    ),
    race0 = dplyr::case_when(
  ...
  dplyr::mutate(
    sex  = factor(sex0,  levels = c("Male","Female")),
    race = factor(race0, levels = c("NH White","NH Black",
  ...
  dplyr::select(SDMVPSU, SDMVSTRA, WTMEC2YR,
                diabetes_ind, age, bmi, age_c, bmi_c, sex, race, race3) %>%
  as.data.frame()

# Guardrails: ensure atomic columns (not list-cols)
stopifnot(!is.list(adult$diabetes_ind),
          !is.list(adult$sex),
          !is.list(adult$race3))
```

```{r}
#| label: adult-type-fixes
#| echo: true
#| message: false
#| warning: false

# Make sure all survey & analysis fields are atomic (no list-cols)
adult <- adult %>%
  mutate(
    diabetes_ind = as.integer(diabetes_ind),
    sex         = factor(as.character(sex)),
    race3       = factor(as.character(race3)),
    WTMEC2YR    = as.numeric(WTMEC2YR),
    SDMVPSU     = as.integer(SDMVPSU),
    SDMVSTRA    = as.integer(SDMVSTRA)
  )

# Guardrails (fail fast if anything is still a list)
stopifnot(!is.list(adult$sex),
          !is.list(adult$race3),
          !is.list(adult$diabetes_ind),
          !is.list(adult$WTMEC2YR))
```

```{r}
#| label: qc-missingness
#| echo: true
#| message: false
#| warning: false

qc_vec <- c("RIAGENDR","RIDRETH1","DIQ010","BMXBMI","RIDAGEYR")
cat("NA counts in merged_data:\n")
print(sapply(merged_data[, qc_vec], function(x) sum(is.na(x))))

cat("\nNA counts in ADULT cohort (post-filter):\n")
print(sapply(adult[, c("sex","race","race3","diabetes_ind","bmi","age")], function(x) sum(is.na(x))))

# Cross-tabs to spot patterned missingness
cat("\nRIDRETH1 missing by RIAGENDR (merged_data):\n")
print(with(merged_data, table(is.na(RIDRETH1), RIAGENDR, useNA = "ifany")))

cat("\nsex missing by race3 (adult):\n")
print(with(adult, table(is.na(sex), race3, useNA = "ifany")))
```
...
groups: White, Black, and Hispanic, with remaining low-frequency levels
combined as Other when necessary.
...
options(survey.lonely.psu = "adjust")  # set once
...
# Guardrails: diabetes_ind must be plain 0/1 numeric for svymean

stopifnot(is.numeric(adult$diabetes_ind))

des_outcome <- subset(nhanes_design_adult, !is.na(diabetes_ind))

overall_est <- survey::svymean(~diabetes_ind, des_outcome, na.rm = TRUE)
overall_ci  <- confint(overall_est)[1, ]
overall_se  <- SE(overall_est)[1]

cat("\n--- Overall Diabetes Prevalence ---\n")
print(data.frame(
Prev = as.numeric(coef(overall_est)[1]),
SE   = overall_se,
LCL  = overall_ci[1],
UCL  = overall_ci[2]
))

by_prevalence <- function(design, factor_name) {
fac  <- design$variables[[factor_name]]
keep <- !is.na(fac) & !is.na(design$variables$diabetes_ind)
fac  <- droplevels(factor(fac[keep]))
if (nlevels(fac) < 1L) return(
data.frame(Level = character(), Prev = numeric(), SE = numeric(), LCL = numeric(), UCL = numeric())
)
levs <- levels(fac)
rows <- lapply(levs, function(L) {
dL  <- subset(design, !is.na(diabetes_ind) & !is.na(.data[[factor_name]]) & .data[[factor_name]] == L)
est <- survey::svymean(~diabetes_ind, dL, na.rm = TRUE)
ci  <- confint(est)[1, ]
data.frame(Level = L,
Prev  = as.numeric(coef(est)[1]),
SE    = SE(est)[1],
LCL   = ci[1],
UCL   = ci[2])
})
...
by_sex_tbl <- by_prevalence(nhanes_design_adult, "sex")
print(by_sex_tbl)

cat("\n--- Diabetes Prevalence by race3 ---\n")
by_race3_tbl <- by_prevalence(nhanes_design_adult, "race3")
print(by_race3_tbl)
```
...
# BMI
p_bmi <- adult %>%
  filter(!is.na(diabetes_ind), !is.na(bmi)) %>%
  mutate(diabetes_f = factor(diabetes_ind, levels = c(0,1), labels = c("No","Yes"))) %>%
...
# Age
p_age <- adult %>%
  filter(!is.na(diabetes_ind), !is.na(age)) %>%
  mutate(diabetes_f = factor(diabetes_ind, levels = c(0,1), labels = c("No","Yes"))) %>%
...
We model a binary outcome Y∈{0,1}Y\in\{0,1\}Y∈{0,1} using **Bayesian
logistic regression**, a member of the generalized linear model (GLM)
family with logit link. The approach combines a likelihood for the
observed binary responses with prior distributions on regression
coefficients. Inference proceeds via posterior simulation (e.g.,
NUTS/HMC), yielding full distributions for parameters and predictive
quantities rather than point estimates alone.

## Model Specification

Let $p_i = \Pr(Y_i = 1 \mid \mathbf{x}_i)$.  
The model is:
...
where continuous predictors may be standardized (mean 0, SD 1) to improve computation and interpretability (per-SD odds ratios), and categorical predictors are coded with reference levels.  
Odds for the event vs. non-event follow:
...
We use weakly informative priors that stabilize estimation while
remaining minimally influential in moderate samples. A common default is

-   coefficients:
    βj∼N(0, 2.52)\beta_j \sim \mathcal{N}(0,\,2.5^2)βj​∼N(0,2.52) (weakly
    informative shrinkage),

-   intercept:
    β0∼Student-t(ν=3,μ=0,σ=10)\beta_0 \sim \text{Student-}t(\nu=3,\mu=0,\sigma=10)β0​∼Student-t(ν=3,μ=0,σ=10).

These choices regularize estimates, mitigate separation, and aid
convergence while preserving interpretability as log-odds effects.

## Bayesian Computation

Posterior inference uses Markov chain Monte Carlo (MCMC), typically
Hamiltonian Monte Carlo with the No-U-Turn Sampler (NUTS). Multiple
chains are run with warmup/adaptation, and tuning parameters (e.g.,
`adapt_delta`) are adjusted to reduce divergent transitions.

## Diagnostics & Model Checking

Convergence and accuracy are assessed via:

-   R^\hat{R}R\^ near 1.00 and large effective sample sizes (ESS),

-   trace plots without drift,

-   Monte Carlo standard errors (MCSE) small relative to posterior SD,

-   posterior predictive checks (e.g., outcome distribution, mean,
    calibration-style plots),

-   summary fit indices (e.g., Bayesian R2R^2R2).

Sensitivity to prior strength is examined by widening/narrowing prior
scales and confirming stability of substantive conclusions.

## Missing Data Strategy

When predictors contain missingness, multiple imputation (e.g., MICE)
can be used to create plausible completed datasets under MAR
assumptions. Analyses are run within each completed dataset and combined
(Rubin’s rules) or fully integrated in a joint Bayesian model.
Categorical predictors should retain at least two observed levels
post-imputation; if a factor collapses to a single level, it is omitted
or further coarsened to avoid separation.

## Handling Rare Categories & Separation

To reduce sparsity, low-frequency factor levels can be coarsened
(lumped) into interpretable groups prior to modeling. Weakly informative
priors already help mitigate quasi- or complete separation; penalized
likelihood (e.g., Firth) is a frequentist alternative for comparison.

## Design/Weights

In complex surveys, design elements (strata, PSUs, weights) can be
addressed by:

1.  **Design-based** modeling (survey-weighted GLM) for frequentist
    comparison, and/or

2.  **Model-based** Bayesian analysis with importance weights
    (normalized survey weights) to approximate representativeness, with
    a note that design-based variance is not fully reproduced unless
    strata/PSUs are modeled explicitly (e.g., hierarchical structures).

## Reporting

Report posterior medians/means with 95% credible intervals for
βj\beta_jβj​, and odds ratios via exp⁡(βj)\exp(\beta_j)exp(βj​) with 95%
CrI. For standardized predictors, interpret effects per 1-SD increase.
Provide posterior predictive summaries and calibration visuals. Include
convergence metrics (R^\hat{R}R\^, ESS) and prior sensitivity results.

## Software & Reproducibility

Implementation can be done in **R** (e.g., `brms`/Stan) or equivalent
probabilistic programming frameworks. Set seeds, record versions, and
share code/scripts to ensure reproducibility.
...
BMI and Age are standardized (mean 0, SD 1). Categorical predictors use
Male and White as reference levels, and race/ethnicity is modeled with
the coarsened three-level factor race3 (White, Black, Hispanic;
rare/other levels lumped as “Other” as needed) to reduce sparsity and
separation. All analyses respect NHANES design elements (PSU, strata,
exam weights).
...
$$
\text{diabetes\_ind} \sim \text{age\_c} + \text{bmi\_c} + \text{sex} + \text{race3}
$$
...
variables and polytomous/logistic regression for categoricals (sex,
race3). The outcome was not imputed; NHANES design variables were
included as auxiliaries only. Because certain imputed datasets
occasionally collapsed a factor to a single level, we fit the logistic
model only on imputations where each included factor retained ≥2 levels,
selected a consistent right-hand side, and pooled those estimates with
Rubin’s rules.

## Bayesian Logistic Regression

We fit the same formula in `brms` with weakly informative priors
(Normal(0, 2.5) for coefficients; Student-t(3, 0, 10) for the intercept,
parameterized as *Student-t(ν = 3, μ = 0, σ = 10)* [@vandeschoot2013]),
sampling with NUTS (4 chains × 2000 iterations; `adapt_delta = 0.95`).
NHANES exam weights were normalized and used as importance weights,
approximating—but not fully reproducing—design-based variance. To
mitigate sparsity, we used race3 and adopted an adaptive formula: if a
factor collapsed to one level in the selected imputed dataset, that
factor was omitted from the Bayesian model. We report posterior ORs with
95% credible intervals for terms retained in the final formula.
...
stopifnot(sum(!is.na(adult$diabetes_ind)) > 0)
...
keep_cc <- with(adult, !is.na(diabetes_ind) & !is.na(age_c) & !is.na(bmi_c) &
...
fit_one_imp <- function(i) {
  ...
  fml <- reformulate(rhs_terms, response = "diabetes_ind")

  # Keep only rows where included factors are not NA
  keep <- !is.na(dat$age_c) & !is.na(dat$bmi_c) & !is.na(dat$diabetes_ind)
...
knitr::kable(
  mi_or,
  caption = sprintf(
    "MICE pooled odds ratios (per 1 SD) — references: Male; White (race3). Pooled across %d/%d imputations; formula: diabetes_ind ~ %s",
...
# Final formula
fml_bayes <- as.formula(paste("diabetes_ind | weights(wt_norm) ~", paste(rhs, collapse = " + ")))
...
Across survey-weighted MLE, MICE-pooled, and Bayesian models, age and
BMI showed strong, positive associations with diabetes risk. Odds ratios
(per 1 SD increase) were broadly consistent across all three frameworks,
indicating stable inference despite missing data and model specification
differences. For example, in the Bayesian model, the odds ratio for BMI
was `r or_text$bay_bmi`, and for Age was `r or_text$bay_age`.

When retained, female sex tended to have lower odds of diabetes compared
with males, while Black and Hispanic adults had higher odds relative to
White. These effects were estimated using the coarsened race3 factor
(White, Black, Hispanic; rare categories lumped as Other) to reduce
sparsity and separation issues.
...
Table summaries and figures confirmed that all models identified BMI and
Age as the most influential predictors, with overlapping credible and
confidence intervals, supporting the robustness and consistency of
results across frequentist and Bayesian approaches.
...
NHANES dataset reinforces well-established findings that higher BMI and
older age are dominant risk factors for diabetes, while sex and
race/ethnicity introduce additional variation in risk. The close
correspondence between the survey-weighted, MICE-pooled, and Bayesian
results supports the robustness of these associations across analytic
frameworks.

Methodologically, the survey-weighted model incorporated NHANES design
elements (strata, clusters, weights), providing
...
The coarsened race3 variable ensured stable estimation, with Black and
Hispanic participants exhibiting higher diabetes odds than White adults,
consistent with known disparities in U.S. diabetes prevalence. The
posterior predictive checks and Bayesian $R^2$ indicated adequate model
fit and predictive validity.

From a translational standpoint, posterior predictions can be extended
to individualized diabetes risk assessment—helping identify BMI
...
2.  The study used NHANES 2013–2014, representing the U.S. adult
    population at that time. Results should be generalized cautiously to
    other countries, populations, or more recent survey waves.

3.  Although multiple imputation (MICE) mitigated missing data, it
    assumes missing at random (MAR); violations such as MNAR could bias
    estimates [@vanbuuren2012].

4.  The Bayesian model used normalized weights rather than explicitly
    modeling strata and PSUs, meaning its uncertainty is model-based and
    may not fully capture design-based variance.

5.  BMI was measured once and may not reflect long-term adiposity or
    metabolic history.

6.  The analysis included a limited covariate set, excluding behavioral,
    socioeconomic, and genetic factors that may confound associations.

7.  Coarsening and adaptive omission of factors. Race/ethnicity was
    simplified to White, Black, and Hispanic (`race3`) to ensure
    stability; in cases where a factor collapsed to one level, it was
    omitted. This reduces granularity and may slightly attenuate group
    contrasts.

8.  The Bayesian model was fit on a single valid imputed dataset, with
    sensitivity checks showing similar age and BMI effects; however,
    credible intervals may be slightly narrower than those from a fully
    integrated multiple-imputation Bayesian approach.
...
#| fig-cap: "Pearson correlations among diabetes_ind (binary), age, and BMI in adult_imp1."
#| fig-width: 6
#| fig-height: 4
...
corr_vars <- subset(adult_imp1, select = c(diabetes_ind, age, bmi))
...
**Correlation Note:** The weighted Pearson correlation between age and
BMI was `r r_age_bmi_chr`.

## 2. Posterior Mean Probability vs BMI (Bayesian)
...
## 3. Priors vs Posteriors (Key Coefficients)
...
ggplot2::ggplot(adult_plot, aes(x = pred_prob, y = as.numeric(diabetes_ind))) +
...
## 5. Posterior vs Survey-Weighted Prevalence

```{r}
#| label: fig-prevalence-compare
#| fig-cap: "Comparison of survey-weighted diabetes prevalence vs Bayesian posterior prevalence."
#| fig-width: 7
#| fig-height: 4

# --- Design-based (survey) prevalence + 95% CI, using the unified design ---

des_outcome <- subset(nhanes_design_adult, !is.na(diabetes_ind))

svy_mean <- svymean(~diabetes_ind, des_outcome, na.rm = TRUE)
pop_prev <- as.numeric(coef(svy_mean))
pop_se   <- as.numeric(SE(svy_mean))
pop_ci   <- c(pop_prev - 1.96 * pop_se, pop_prev + 1.96 * pop_se)
pop_ci   <- pmin(pmax(pop_ci, 0), 1)

# --- Model-based (Bayesian) posterior prevalence + 95% CrI ---

# Predict on the SAME data used to fit (adult_imp1) to avoid level/NA issues

bayes_pred <- posterior_epred(bayes_fit, newdata = adult_imp1)   # draws x N
post_prev_draw <- rowMeans(bayes_pred)                           # one prevalence per draw
post_prev      <- mean(post_prev_draw)
post_ci        <- quantile(post_prev_draw, c(0.025, 0.975))

stopifnot(is.finite(post_prev))
