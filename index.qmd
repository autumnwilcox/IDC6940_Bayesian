...
all_tbl <- bind_rows(svy_tbl, bayes_tbl) %>%
  mutate(
    term = case_when(
      ...
      grepl("Other", term, ignore.case = TRUE)  ~ "Other (vs. White)",
      TRUE ~ term
    ),
...
Across both frameworks—survey-weighted (MLE) and Bayesian—age and BMI were 
...
highly consistent with the survey-weighted design-based model.
...
identified consistent predictors of diabetes risk among U.S. adults: 
advancing age, higher BMI, sex (lower odds for females), and non-White 
race/ethnicity. These convergent findings reinforce the robustness of 
core risk factors and the validity of the analytic approach across modeling 
paradigms.
