...
The following histograms and bar charts describe the adult analytic cohort. Most participants are middle-aged, with a normal-to-overweight BMI distribution and majority White race composition. This visualization helps contextualize the predictors used in the models.
...
The design-based model accounts for NHANES sampling weights and clustering. Positive coefficients for age and BMI indicate higher diabetes odds per one-SD increase, while categorical contrasts show elevated odds among non-White groups.
...
Weakly informative priors ($N(0, 2.5)$ for slopes; Student-t(3, 0,
...
Bayesian parameter estimates are consistent with frequentist results, with credible intervals providing direct probability statements. Diagnostics show good chain mixing ($\hat{R} \approx 1.00$) and effective sample sizes exceeding standard thresholds, confirming convergence.
...
Leave-one-out (LOO) cross-validation indicates that removing race or sex reduces model fit, confirming that both variables meaningfully contribute to predictive performance.
