...
Notes: Reference categories are **Male** for sex and **White** for race3. **Age** and **BMI** ORs are per **1 SD** increase.
...
The multiple imputation (MICE) framework addressed missingness in **BMI** using predictive mean matching (PMM). **Diabetes status (`diabetes_ind`) was not imputed** (used as a predictor only). Categorical variables (`sex`, `race3`) used logistic/polytomous regression. We generated m = 5 imputations and pooled estimates with Rubin's rules. 
