...
and Nutrition Examination Survey (NHANES). Predictors include age, body mass index (BMI), sex, and a coarsened race/ethnicity factor (`race3`) comprising White, Black, and Hispanic groups, with low-frequency levels combined as “Other.” The NHANES race/ethnicity source variable (`RIDRETH1`) includes Mexican American, Other Hispanic, Non-Hispanic White, Non-Hispanic Black, and Other/Multi. For analysis, we collapsed these into four groups—White, Black, Hispanic, and Other—to ensure adequate representation and model stability.

Three analytic frameworks were compared: (1) survey-weighted maximum likelihood estimation (MLE) using the NHANES complex design, (2) multiple imputation (MICE) with predictive mean matching and Rubin’s rules, and (3) Bayesian inference with weakly informative priors $N(0, 2.5)$ implemented via `brms`. The Bayesian
...
# Diabetes by race3 (dodged bars)
ggplot(adult, aes(x = race3, fill = factor(diabetes_ind, levels = c(0,1), labels = c("No","Yes")))) +
  geom_bar(position = "dodge") +
  labs(title = "Diabetes Diagnosis by Race/Ethnicity (≥20 years)", x = "Race/Ethnicity (race3)", y = "Count", fill = "Diabetes") +
  theme_minimal() +
...
Three modeling frameworks were compared using identical predictors (standardized age, BMI, sex, and race3) and the binary outcome `diabetes_ind`:
...
#| message: false
#| warning: false
#| dependson: [adult-imp1, define-bayes-formula]
#| cache: false
...
to_model_row <- function(age_raw, bmi_raw, sex_lab, race3_lab) {
  tibble(
    ...
    race3  = factor(race3_lab, levels = levels(adult_imp1$race3)),
    wt_norm = 1
  )
}
...
p1 <- to_model_row(adult$age[1], adult$bmi[1],
                   as.character(adult$sex[1]), as.character(adult$race3[1]))
p2 <- to_model_row(adult$age[2], adult$bmi[2],
                   as.character(adult$sex[2]), as.character(adult$race3[2]))
...
new_pt <- to_model_row(age_raw = 45, bmi_raw = 32, sex_lab = "Female", race3_lab = "Hispanic")
...
want <- c("b_age_c","b_bmi_c","b_sexFemale","b_race3Black","b_race3Hispanic","b_race3Other")
...
#| label: bayes-yobs
#| echo: false
#| dependson: [adult-imp1, fit-bayes]
#| cache: false
...
want_post <- c("b_age_c","b_bmi_c","b_sexFemale","b_race3Black","b_race3Hispanic","b_race3Other")
...
b_race3Black = "Black vs White",
b_race3Hispanic = "Hispanic vs White",
b_race3Other = "Other vs White"
)
